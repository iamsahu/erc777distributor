...
contract ERC777Distributor is IERC777Recipient,SuperAppBase {
    ...
    event UserAdded(address userAddress,uint128 shares,uint32 index,address publisher,uint timeStamp);
    event UserRemoved(address userAddress,uint128 shares,uint32 index,address publisher,uint timeStamp);
    event UserModified(address userAddress,uint128 shares,uint32 index,address publisher,uint timeStamp);
    event Distribution(uint256 amountDistributed,uint32 index,address publisher,uint timeStamp);
    ...
    function addUser(address newUser,uint128 sharePercentage) external onlyOwner{
        ...
        emit UserAdded(newUser,shareUnits,INDEX_ID,address(this),block.timestamp);
    ...
    function modifyUser(address existingUser,uint128 sharePercentage) external onlyOwner{
        ...
        emit UserModified(existingUser,shareUnits,INDEX_ID,address(this),block.timestamp);
        emit TotalShares(totalShareUnits,INDEX_ID,address(this),block.timestamp);
    }
    ...
    function removeUser(address existingUser) external onlyOwner{
        ...
        emit UserRemoved(existingUser,temp,INDEX_ID,address(this),block.timestamp);
        emit TotalShares(totalShareUnits,INDEX_ID,address(this),block.timestamp);
    }
    ...
    function distribute(uint256 cashAmount,address tokenAddress) internal {
        ...
        emit Distribution(cashAmount,INDEX_ID,address(this),block.timestamp);
    }
